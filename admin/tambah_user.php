...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($customer_name)) {
        ...
        // 4. Gunakan Prepared Statements untuk keamanan
        $sql = "INSERT INTO customers (api_key, customer_name, email, daily_request_limit, is_active, email_check_server) VALUES (?, ?, ?, ?, ?, ?)";
        
        if ($stmt = mysqli_prepare($conn, $sql)) {
            // 5. Bind variabel ke prepared statement sebagai parameter
            mysqli_stmt_bind_param($stmt, "ssisis", $api_key, $customer_name, $email, $daily_request_limit, $is_active, $email_check_server);
            
            // 6. Eksekusi statement
            if (mysqli_stmt_execute($stmt)) {
                // Jika berhasil, alihkan kembali ke halaman utama
                header("Location: email_validation.php?status=sukses_tambah");
                exit();
            } else {
