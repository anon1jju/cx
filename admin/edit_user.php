...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Jika input email tidak kosong, gunakan nilainya. Jika kosong, set sebagai NULL.
...
// --- AKHIR PERBAIKAN ---
    ...
    $email_check_server = mysqli_real_escape_string($conn, $_POST['email_check_server']); // <-- Tambahkan ini
    ...
    if (!empty($customer_name)) {
        ...
        $sql_update = "UPDATE customers SET customer_name = ?, email = ?, daily_request_limit = ?, is_active = ?, email_check_server = ? WHERE id = ?";
        
        if ($stmt_update = mysqli_prepare($conn, $sql_update)) {
            // Bind parameter
            mysqli_stmt_bind_param($stmt_update, "ssiisi", $customer_name, $email, $daily_request_limit, $is_active, $email_check_server, $user_id);
            
            // Eksekusi
            if (mysqli_stmt_execute($stmt_update)) {
                // Jika berhasil, redirect ke halaman utama dengan pesan sukses
                header("Location: email_validation.php?status=sukses_update");
                exit();
            } else {
        ...
        $pesan_error = "Nama Customer dan Email tidak boleh kosong.";
    }
                            ...
                            <form action="edit_user.php" method="POST">
                                ...
                                <div class="mb-6">
                                    ...
                                    <select id="email_check_server" name="email_check_server"
                                        ...
                                        <option value="check.php" <?php if($user['email_check_server'] == 'check.php') echo 'selected'; ?>>Server 1 (Utama)</option>
                                        <option value="check2.php" <?php if($user['email_check_server'] == 'check2.php') echo 'selected'; ?>>Server 2 (Alternatif)</option>
                                        <option value="check3.php" <?php if($user['email_check_server'] == 'check3.php') echo 'selected'; ?>>Server 3 (Cadangan)</option>
                                        <option value="check4.php" <?php if($user['email_check_server'] == 'check4.php') echo 'selected'; ?>>Server 4 (Cadangan)</option>
                                        <option value="check5.php" <?php if($user['email_check_server'] == 'check5.php') echo 'selected'; ?>>Server 5 (Cadangan)</option>
                                    </select>
                                </div>
                                <div class="mb-6">
                                ...
                                <div class="flex items-center justify-end">
                                    <a href="email_validation.php" class="text-gray-600 hover:text-gray-800 font-medium mr-4">Batal</a>
                                    <button type="submit"
